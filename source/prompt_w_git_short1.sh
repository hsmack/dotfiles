...
#
# this doesn't show up in the prompt
# needs to be loaded in $PROMPT_COMMAND
# http://stackoverflow.com/questions/945288/saving-current-directory-to-bash-history
#
function history_pwd() {
...
#
# builds the prompt to be run with $PROMPT_COMMAND
#
function prompt_command() {
	#
	# functions to be executed too
	#
    # HISTORYPWD is best used for servers, where changing dirs remotely is painfully slow
    if [[ $HISTORYPWD ]];
    then
    	history_pwd
    fi
	...
	# path: [@host:path user]
	#PS1="$PS1$c1[$c0\h$c1:$c0\w$c1 $c0\u$c1]$c9"
